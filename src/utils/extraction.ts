  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (enrollmentFound && !nameFound) {
      ...
      if (lowerLine.includes('d/o') || lowerLine.includes('s/o') || lowerLine.includes('w/o')) {
        for (let j = i - 1; j >= 0; j--) {
          ...
          if (isValidNameLine(prevLine)) {
            console.log('Found name before relation line:', prevLine);
      ...
      if (isValidNameLine(line)) {
        console.log('Found potential name after enrollment:', line);
        return line.trim();
      }
    }
  }
...
export function extractAadhaarNumber(text: string): string {
...
export function extractDateOfBirth(text: string): string {
